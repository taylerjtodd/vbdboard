...
export function calculateVbd(
  ...
  positions.forEach((pos) => {
    ...
    projList.forEach((proj) => {
      ...
      if (pos === 'dst') {
        rankItem = rankList.find(
          (r) =>
            r.pos === pos &&
            r.name.substring(0, 6) === proj.name.substring(0, 6)
        );
      } else {
        rankItem = rankList.find((r) => r.pos === pos && r.name === proj.name);
      }

      const draftedIdx = draftedPlayers.findIndex((dp, i) => {
        if (pos === 'dst') {
          return (
            dp.pos === pos &&
            dp.name.substring(0, 6) === proj.name.substring(0, 6)
          );
        }
        return dp.pos === pos && dp.name === proj.name;
      });
